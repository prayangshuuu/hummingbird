...
static int find_free_slot(hbi_memory_manager_t* mm) {
    for (int i = 0; i < MAX_BLOCKS; i++) {
        if (!mm->blocks[i].allocated) return i;
    }
    return -1;
}

bool hbi_memory_manager_allocate(hbi_memory_manager_t* mm, hbi_block_id_t id, size_t size, hbi_memory_tier_t tier) {
    if (!mm) return false;
    if (find_block(mm, id) >= 0) return false; // Already allocated
    if (mm->tier_used[tier] + size > mm->tier_capacities[tier]) return false; // OOM
...
hbi_residency_status_t hbi_memory_manager_get_residency(hbi_memory_manager_t* mm, hbi_block_id_t id) {
    if (!mm) return HB_RESIDENCY_EVICTED;
    int slot = find_block(mm, id);
    if (slot < 0) return HB_RESIDENCY_EVICTED;
    return mm->blocks[slot].status;
}

bool hbi_memory_manager_move(hbi_memory_manager_t* mm, hbi_block_id_t id, hbi_memory_tier_t new_tier) {
    if (!mm) return false;
    int slot = find_block(mm, id);
    if (slot < 0) return false;
