...
/* Error Translation */
static hbi_status translate_cuda_error(cudaError_t err, const char *file, int line,
...
static hbi_status cuda_get_capabilities(hbi_backend_capabilities *out_caps) {
    ...
    out_caps->supported_datatypes = HBI_BACKEND_DTYPE_F32 | HBI_BACKEND_DTYPE_F16 |
                                    HBI_BACKEND_DTYPE_BF16 | HBI_BACKEND_DTYPE_I8 |
                                    HBI_BACKEND_DTYPE_I4;
...
static hbi_status cuda_create_context(hbi_allocator *allocator, hbi_backend_context **out_ctx) {
    ...
    if (!ctx)
        return HBI_ERR_SET(HBI_ERR_OOM, 0, "Failed to allocate CUDA context");
...
static hbi_status cuda_execute(hbi_backend_context *ctx, const hbi_backend_command *cmd) {
    ...
    case HBI_CMD_KERNEL_DISPATCH: {
        if (!cmd->params.dispatch.kernel_descriptor)
            return HBI_ERR_SET(HBI_ERR_INVALID_ARG, 0, "CUDA dispatch NULL kernel");
    ...
    case HBI_CMD_MEMCOPY_H2D: {
        CUDA_CHECK(cudaMemcpyAsync(cmd->params.copy.dst, cmd->params.copy.src,
                                   cmd->params.copy.bytes, cudaMemcpyHostToDevice, ctx->stream),
                   "H2D");
    ...
    case HBI_CMD_MEMCOPY_D2H: {
        CUDA_CHECK(cudaMemcpyAsync(cmd->params.copy.dst, cmd->params.copy.src,
                                   cmd->params.copy.bytes, cudaMemcpyDeviceToHost, ctx->stream),
                   "D2H");
    ...
    case HBI_CMD_MEMCOPY_D2D: {
        CUDA_CHECK(cudaMemcpyAsync(cmd->params.copy.dst, cmd->params.copy.src,
                                   cmd->params.copy.bytes, cudaMemcpyDeviceToDevice, ctx->stream),
                   "D2D");
    ...
    default:
        return HBI_ERR_SETF(HBI_ERR_UNSUPPORTED, 0, "CUDA backend: unsupported command type %d",
                            (int)cmd->type);
    }
}
...
static const hbi_backend g_cuda_backend = {HBI_BACKEND_ABI_VERSION,
                                           "cuda",
                                           cuda_init,
                                           cuda_shutdown,
