...
int main() {
    ...
    // Actually the backend might crash if allocator is NULL. Let's just pass NULL if it tolerates it
    // Our implementation checks !allocator, so it will fail. We need a dummy allocator.
    // For simplicity, we just won't test context creation if we don't have a real allocator,
    // or we can implement a dummy one.
